from predictionedge.sizing import (
...
def test_daily_loss_limit_is_five_percent():
    assert Account(size=50_000).daily_loss_limit == 2_500


def test_size_position_respects_risk_budget():
    a = Account(size=100_000, per_trade_fraction=0.01)   # $1,000 budget
    s = size_position(0.50, a, conviction=1.0)
    assert s is not None
    assert s.contracts == 1900          # budget wants 2000, house caps at 19/$1k
    assert s.capped_by == "per-market-limit"


def test_conviction_scales_size_down():
    a = Account(size=100_000, per_trade_fraction=0.001)  # $100 budget
...
def test_event_ceiling_accounts_for_existing_exposure():
    a = Account(size=1_000, per_trade_fraction=1.0)
...
def test_rejects_sub_penny_and_resolved_prices():
    ...
    assert size_position(0.005, a) is None      # below the $0.01 minimum
    assert size_position(1.0, a) is None


def test_rejects_when_budget_buys_nothing():
    a = Account(size=100, per_trade_fraction=0.0001)   # $0.01 budget
